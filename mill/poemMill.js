...
//const bleed=0.125;
const spine=0.3;
const coversvgwidth = (w*2+bleed*2+spine)*pixelsperunit; 
const coversvgheight = (h+bleed*2)*pixelsperunit;
const markup = str => {
...
const covercanvas = {width:coversvgwidth,height:coversvgheight,min:Math.min(coversvgwidth,coversvgheight),max:Math.max(coversvgwidth,coversvgheight)};
//let coversobj = [...new Array(4).keys()].map( j => j*8).flatMap( j => { 
let coversobj = [...new Array(nticks).keys()].map( j => {
	...
	let elementdraw = B.elements.map( layer => {
		return layer.map( el => {
			...
			//console.log(JSON.stringify(p));
			//console.log(JSON.stringify(tools.drawf(canvas,p,el.tag)));
			return tools.drawf(covercanvas,p,el.tag);
		}).join(" ");
	}).join(" ");
	cover.figure = {
		picture:`
	<svg viewBox="0 0 ${coversvgwidth} ${coversvgheight}">
		${elementdraw}
	</svg>
	`,
		caption: "",
...
bookobj.dt = millinfo.dt;
bookobj.datetime = millinfo.datetime;
bookobj.sections = [
	{ 
		id: "section1",
		title: "the repair(*)",
...
filmobj.dt = millinfo.dt;
filmobj.datetime = millinfo.datetime;
filmobj.sections = [
	{ 
		id: "title",
		title: "field notes",
...
let coversstr = `let poems =
	${JSON.stringify(coversobj,null,"\t")};
module.exports = poems;`
